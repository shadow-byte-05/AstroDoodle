    ...
    function stopDrawing(e) {
        ...
        objStartX = minX - currentSize
        objStartY = minY - currentSize
        objWidth = maxX - minX + currentSize * 2
        objHeight = maxY - minY + currentSize * 2
        const relativePath = new Path2D()
        relativePath.moveTo(
            pathPoints[0].x - objStartX,
            pathPoints[0].y - objStartY
        )
        pathPoints.forEach((p) => {
            relativePath.lineTo(p.x - objStartX, p.y - objStartY)
        })
    ...
    document.getElementById('velocity-slider').addEventListener('input', (e) => {
        ...
        parseFloat(baseVelocity).toFixed(1)
    })
