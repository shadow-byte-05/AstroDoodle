window.addEventListener('load', () => {
    ...
    function stopDrawing(e) {
        if (!isDrawing) return;
        isDrawing = false;
        // For touchend, position is in changedTouches
        const pos = e.changedTouches ? getMousePos(e.changedTouches[0]) : getMousePos(e);
        ...
        if (currentTool === 'brush' || currentTool === 'eraser') {
            if (pathPoints.length < 2) { // Ignore clicks
                pathPoints = [];
                currentPath = null;
                return;
            }
            ...
            pathPoints.slice(1).forEach(p => { relativePath.lineTo(p.x - objStartX, p.y - objStartY); });
